...
class _PgConn:
    ...
    def execute(self, sql, params=()):
        if USE_PG:
            # Convert SQLite-compatible SQL to PostgreSQL before execution.
            # This is important for analytics and export queries because the
            # application stores timestamps as TEXT for SQLite compatibility.
            sql = _to_pg(sql)
            # INSERT OR IGNORE → INSERT ... ON CONFLICT DO NOTHING
            if "INSERT OR IGNORE" in sql.upper():
                import re as _re
                sql = _re.sub(r"(?i)INSERT OR IGNORE INTO", "INSERT INTO", sql)
                sql = sql.rstrip()
